...
写作质量要求（务必遵守）：
- 使用规范、自然的书面中文。front 是一个明确、有思考价值的问题或概念名，不使用「是什么」式的空泛问法，尽量结合真实情境。
- back 用 70-160 字完整作答：先给结论，再解释原因或机制，必要时给例子、边界条件或可操作做法，读完能真正学会，而不是只记住一个名词。
- concept 卡的 back 要包含定义、关键特征和一个生活化例子；scenario 卡要给出分析步骤与参考做法。
- 每张卡片只讲一个知识点，但信息密度要足够，禁止一两句话敷衍；语言要像耐心的老师在讲解。
