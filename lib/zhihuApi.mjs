...
export async function fetchOAuthUser(oauthToken) {
  ...
  const urlToken = String(source.url_token || source.UrlToken || source.urlToken || '').trim();
  const url = normalizePublicUrl(source.url || source.Url || source.link || source.Link || (urlToken ? `https://www.zhihu.com/people/${urlToken}` : ''));
